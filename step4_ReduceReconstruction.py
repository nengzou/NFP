...
# 数据文件夹路径
data_folder = "H:\Project/firstArtical\data/liangKuai"
output_filename = "reducefusiontest.ply"#在data_folder下新建一个结果
N = 12
n = 4
num = n + 2 + 1
...
points_list = []
# 从上到下，从左到右，进行搜索
for Y_pha in range(combi_size//2, x_p.shape[0] - combi_size//2):  # 从第2行开始，到600-1行

    for X_pha in range(combi_size//2, x_p.shape[1] - combi_size//2):  # 从第2列开始到1400-1列
        # 构造融合框
        roi_L = (X_pha - combi_size//2, Y_pha - combi_size//2, combi_size, combi_size)  # 范围的左上点坐标，窗口大小
        ...
        if non_nan_count > 8:#符合要求9个点都有效，进行融合修正
            ...
            theta = np.degrees(theta)
            #确定是否保留点
            if theta > 20 :#大于20跳出循环，不保存点
                print("Reduced")
                continue

            points_list.append(BOX_xyz[1,1])#存储融合点
